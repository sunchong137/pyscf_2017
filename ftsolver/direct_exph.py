...
def spectrum_time(t,g,hpp,nsite,nexciton,nmode,nphonon,time_step,T,nsamp=20000,m=40,**kwargs):
    def hop(v):
        return contract_all(t,g,hpp,v,nsite,nexciton,nmode,nphonon)
    def muop(v):
        return v #FIXME input needed
    ld = make_shape(nsite,nexciton,nmode,nphonon)
    C_t = smpl_ep.smpl_time(hop,muop,T,time_step,ld,nsamp=nsamp,M=m)
    return C_t

def spectrum_freq(t,g,hpp,nsite,nexciton,nmode,nphonon,freq_list,T,nsamp=20000,m=40,**kwargs):
    def hop(v):
        return contract_all(t,g,hpp,v,nsite,nexciton,nmode,nphonon)
    def muop(v):
        return v #FIXME input needed
    ld = make_shape(nsite,nexciton,nmode,nphonon)
    C_w = smpl_ep.smpl_freq(hop,muop,T,freq_list,ld,nsamp=nsamp,M=m)
    return C_w
...
if __name__ == '__main__':
    ...
    T = 0.1
#   ct = spectrum_time(t,g,hpp,nsite,nexciton,nmode,nphonon,time_step,T,nsamp=200,m=10)
#   print ct
    freq_list = (2.0, 10)
    cw = spectrum_freq(t,g,hpp,nsite,nexciton,nmode,nphonon,freq_list,T,nsamp=200,m=10)
    print cw
#   cishape=make_shape(nsite, nexciton, nmode, nphonon)
