...
def ft_ismpl_E(hop, ci0, T, nw=25, nsamp=2000, M=50, dr=0.5, \
    ...
    for i in range(nw):
        ci = gen_nci(ci0, dr=dr, dtheta=dtheta)
...
###############################################################
def ft_ismpl_rdm1s(qud, hop, ci0, T, norb, nw=25, nsamp=100,\
         M=50, dr=0.5, save_prof=False, prof_file=None, \
...
#########################################################################
def ft_ismpl_rdm12s(qud, hop, ci0, T, norb, nw=25, nsamp=100,\
         M=50, dr=0.5, gen_prof=False, prof_file=None,\
         Hdiag=None, verbose=0, **kwargs):
...
#    E = E/(1.*nsamp)
    ...
    if gen_prof: 
        if prof_file is None:
            if not os.path.exists("./data"):
                os.makedirs("./data")
            prof_file = "./data/doccprof_T%2.2f.npy"%T
...
def gen_nci(v0, cases=0, dr=0.5, dtheta=20.):
